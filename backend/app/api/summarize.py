...
async def stream_summary(request: SummarizeRequest):
    ...
    try:
        ...
        if not subtitle_text or len(subtitle_text.strip()) < 10:
            # Try to use title + description as context
            ...
            if title or description:
                ...
                # Stream summary with limited context
...
async def stream_chat(task_id: str, request: ChatRequest):
    """Stream chat response."""
    try:
        # Get task and subtitle
        task = summarize_tasks.get(task_id)
        if not task:
            return StreamingResponse(
                _stream_error_generator(f"Task not found: {task_id}"),
                media_type="text/event-stream"
            )
        ...
        if not subtitle_text:
            return StreamingResponse(
                _stream_error_generator("No subtitle available for this video"),
                media_type="text/event-stream"
            )
